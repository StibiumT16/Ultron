...
parser.add_argument("--data_path", default="docs-sents.json", type=str, help='data path')
parser.add_argument("--docid_path", default=None, type=str, help='docid path')
parser.add_argument("--query_path", default="raw_data/queries.train.tsv", type=str, help='data path')
parser.add_argument("--qrels_path", default="raw_data/qrels.train.tsv.tsv", type=str, help='data path')
...
def add_docid_to_vocab(doc_file_path):
    print(f"data path:{doc_file_path}")
...
def gen_passage_instance(id_to_token, token_to_id, all_docid, encoded_docid):
    tokenizer = T5Tokenizer.from_pretrained(args.pretrain_model_path)
    print("generate passage instance")
    sample_count = 0
    ...
    with open(args.data_path) as fin:
        for doc_index, line in tqdm(enumerate(fin), desc='generating samples'):
            ...
            while sent_id < len(sents_list):
                ...
                sent_id += 1
    fw.close()
    print("total count of passages: ", sample_count)

# 任务1.2：sampled terms --> docid
def gen_sample_terms_instance(id_to_token, token_to_id, all_docid, encoded_docid):
    tokenizer = T5Tokenizer.from_pretrained(args.pretrain_model_path)
    sample_count = 0
    print("generate sampled terms instance")
    print(f"input file:{args.data_path}; output file:{args.output_path}")
    fw = open(args.output_path, "w")
    ...
    fw.close()
    print("total count of sampled terms: ", sample_count)

# 任务2：pseudo query --> docid
def gen_fake_query_instance(id_to_token, token_to_id, all_docid, encoded_docid):
    tokenizer = T5Tokenizer.from_pretrained(args.pretrain_model_path)
    print("generate fake query instance")
    print(f"input file:{args.fake_query_path}; output file:{args.output_path}")
    # 从ms-marco数据集中检索出点击了docid的对应query 
    fw = open(args.output_path, "w")   
    max_num_tokens = args.max_seq_length - 1
        
    with open(args.fake_query_path, "r") as fr:
        for i, line in enumerate(tqdm(fr, desc="load all fake queries")):
            docid, query = line.strip("\n").split("\t")
            assert docid in token_to_id
...
def gen_query_instance(id_to_token, token_to_id, all_docid, encoded_docid):
    tokenizer = T5Tokenizer.from_pretrained(args.pretrain_model_path)
    print("generate query-docid instance")
    print(f"input file:{args.query_path} and {args.qrels_path}; output file:{args.output_path}")
    # 从ms-marco数据集中检索出点击了docid的对应query 
    fw = open(args.output_path, "w")
    qid_2_query = {}
